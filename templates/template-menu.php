...
<main id="primary" class="site-main max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative">
    <header class="mb-16 text-center">
        <h1 class="text-5xl font-extrabold text-gray-900 tracking-tight mb-4"><?php the_title(); ?></h1>
        <div class="prose max-w-2xl mx-auto text-lg text-gray-500">
            <?php the_content(); ?>
        </div>
    </header>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    if ($menu_items->have_posts()) {
                        while ($menu_items->have_posts()) {
                            ...
                            curry_leaves_co_render_menu_item();
                        }
                        wp_reset_postdata();
                    }
                    ?>
     ...
     */
    function curry_leaves_co_render_menu_item()
    {
        ...
        $img_url = get_the_post_thumbnail_url(get_the_ID(), 'large') ?: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=800&q=80';

        $desc_html = apply_filters('the_content', get_the_content());
        $desc_clean = wp_strip_all_tags($desc_html);
        ?>
        <div class="relative group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col">
            ...
            <div class="relative h-56 overflow-hidden bg-gray-100">
                ...
                <?php if ($rating && $rating !== '0.0'): ?>
                    <div
                        class="absolute top-4 right-4 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-full text-sm font-bold text-gray-800 flex items-center gap-1 shadow-sm">
            ...
            <div class="p-6 flex-grow flex flex-col">
                ...
                <div class="mt-auto flex gap-3 pt-5 border-t border-gray-50 relative z-10">
                    <button onclick="event.stopPropagation(); alert('Item added to cart!');"
                        class="flex-1 bg-white border-2 border-orange-600 text-orange-600 hover:bg-orange-50 font-semibold py-2.5 px-4 rounded-xl transition-colors text-sm text-center flex items-center justify-center gap-2">
                    ...
                    <button onclick="event.stopPropagation(); alert('Proceeding to checkout...');"
                        class="flex-1 bg-orange-600 hover:bg-orange-700 text-white font-semibold py-2.5 px-4 rounded-xl transition-colors text-sm text-center">
                        Order Now
                    </button>
                </div>
            </div>
